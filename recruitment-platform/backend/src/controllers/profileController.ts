...
export const updateProfileValidation = [
  ...
  body("gender")
    ...
    .withMessage(
      "Gender must be one of: Male, Female, Non-binary, Prefer not to say, Other"
    ),
  body("dob")
    .optional()
    .isISO8601()
...
export const updateProfile = async (
  ...
  try {
    ...
    // Validate CTC relationship
    if (updates.currentCTC !== undefined && updates.expectedCTC !== undefined) {
      if (updates.expectedCTC < updates.currentCTC) {
        res.status(400).json({
          success: false,
...
export const deleteProfile = async (
  ...
  try {
    ...
    res.json({
      success: true,
      message:
        "Profile deletion request received. This feature can be implemented based on business requirements.",
    } as IApiResponse);
  } catch (error) {
